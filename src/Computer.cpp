...
int Computer::negamax(int depth, int alpha, int beta, int perspective, Board &board) 
{
    ...
    {
        board.makeMove(mv.first);
        int value = -negamax(depth - 1, -beta, -alpha, -perspective, board); // switches and negates alpha and beta since they are now from the perspective of the other player
        board.undoMove();
...
std::vector<std::pair<int, std::vector<int>>> Computer::orderMoves(std::vector<std::pair<int, std::vector<int>>> moves)
{
    std::sort(moves.begin(), moves.end(), [&](const std::pair<int, std::vector<int>> &a, const std::pair<int, std::vector<int>> &b) {
        return (m_weight_map[a.first] + 3 * a.second.size()) > (m_weight_map[b.first] + 3 * b.second.size()); // orders moves based on the position and amount of flips
    });
    return moves;
}
